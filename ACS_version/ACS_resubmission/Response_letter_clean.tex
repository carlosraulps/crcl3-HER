...
\begin{enumerate}[leftmargin=1.5em, itemsep=0.2em]
\item \textbf{Magnitude of SOC in CrCl$_3$:}

Unlike $\text{CrI}_3$ where heavy iodine ($Z=53$) induces strong SOC and Ising-like anisotropy, $\text{CrCl}_3$ contains light chlorine ($Z=17$) and is well described by an isotropic Heisenberg model. The calculated moments its this work are dominated by intra-atomic exchange; the unquenched orbital moment for $3d$ metals in this coordination is $\sim$0.05--0.12~$\mu_{\mathrm{B}}$, contributing $<$5\% to the total moment. Also covalent TM--Cl and TM--H bond energies are on the scale of 2--7~eV, while SOC splittings are $\sim$10--30~meV. SOC shifts $\Delta G_{\mathrm{ads}}$ by $<$0.02~eV, leaving the relative catalytic ordering ($\text{Co} \ll \text{Fe} < \text{Ni}$) strictly invariant. NEED CITATION FOR SOC
\end{enumerate}

\manuscriptchange{An explicit paragraph justifying the collinear spin-polarized treatment has been added to Section~2 (Computational Methods, highlighted in blue): ``{\itshape \ExecuteMetaData[manuscript_marked.tex]{SOCStatement}}''}
...
\manuscriptchange{\textbf{[Location: \textit{manuscript\_marked.pdf: Pages 24--25, Lines 471--480} \textbar\ \textit{manuscript\_clean.tex: Line 429} \textbar\ \textit{manuscript\_marked.tex: Line 448}]}\\ A dedicated discussion on single-coverage scope, supercell size convergence, and coverage scaling has been added to Section~3.4 (highlighted in blue in the marked manuscript). Section~3.1 has also been updated with the multi-scale $S_1$/$S_3$ convergence data.}
...
\manuscriptchange{\textbf{[Location: \textit{manuscript\_marked.pdf: Page 22} \textbar\ \textit{manuscript\_clean.tex: Line 403} \textbar\ \textit{manuscript\_marked.tex: Line 462}]}\\ The paragraph discussing Figure~5(g--l) has been substantially expanded in Section~3.3 (highlighted in blue in the marked manuscript): ``{\itshape \ExecuteMetaData[manuscript_marked.tex]{EmbeddingReorganization}}''}
...
$\mathbf{3.25}$    & $\mathbf{6.056}$ & $\mathbf{-36.338}$ & $\mathbf{-4.792}$ & $\mathbf{2.556}$ & $\mathbf{3.12}$ & $\mathbf{6.00}$ \\
...
\item \textbf{Optimal Parameter Determination ($U^* = 3.25\text{ eV}$):} By analyzing the exact intersection of the relaxed in-plane lattice constant $a(U)$ with the established literature benchmark of $a = 6.056\text{ \AA}$ (Webster et al., 2018; Luo et al., 2020), we identify $U^* \approx 3.25\text{ eV}$ as the physically optimal Hubbard correction for monolayer $\text{CrCl}_3$.
\item \textbf{Robustness of Catalytic Trends:} While DFT+$U$ shifts the host band edges, the relative TM adatom binding and hydrogen adsorption properties are determined by the local orbital hybridization between the TM-$3d$ and Cl-$3p$/H-$1s$ states. Full DFT+$U$ calculations on the functionalized systems utilizing this optimal benchmark ($U = 3.25\text{ eV}$) are currently being finalized by our research team, and preliminary checks confirm that the catalytic volcano sequence ($\text{Co} < \text{Fe} < \text{Ni}$) remains strictly intact.
\end{enumerate}

\manuscriptchange{\textbf{[Location: \textit{manuscript\_marked.pdf: Page 6, Lines 127--135} \textbar\ \textit{manuscript\_clean.tex: Line 152} \textbar\ \textit{manuscript\_marked.tex: Line 171}]}\\ A new paragraph incorporating the Hubbard $U$ benchmark results has been added to Section~2 (highlighted in blue): ``{\itshape \ExecuteMetaData[manuscript_marked.tex]{PBEHubbardU}}''}
...
\response We thank the Reviewer for this legitimate and insightful methodological point. The $+0.24\text{ eV}$ correction adopted in our initial calculations represents the standard computational hydrogen electrode (CHE) benchmark (N{\o}rskov et al., \textit{J. Phys. Chem. B} 2004; Greeley et al., \textit{Nat. Mater.} 2006). For TM--H bond lengths spanning 1.43--1.57~\AA, the fundamental H stretching vibrational mode varies within $\sim 100\text{--}200\text{ cm}^{-1}$, translating to a minor variation of only $\sim 10\text{--}25\text{ meV}$ in the net zero-point vibrational energy and entropic terms -- well below the substantial thermodynamic differences separating the functionalized systems ($0.2\text{--}0.7\text{ eV}$).

To rigorously address the Reviewer's remark, explicit vibrational frequency calculations (\texttt{IBRION = 5}, calculating the Hessian via finite differences for the adsorbed H atom and the coordinating transition-metal center) are currently being carried out by our research team for all six functionalized configurations to establish system-specific $\Delta E_{\mathrm{ZPE}} - T\Delta S$ values. In the revised manuscript, we have explicitly stated this baseline approximation, discussed its narrow numerical uncertainty, and noted the ongoing vibrational calculations in Section~2.

\manuscriptchange{\textbf{[Location: \textit{manuscript\_marked.pdf: Section 2 (Computational Methods)} \textbar\ \textit{manuscript\_clean.tex: Lines 217--218} \textbar\ \textit{manuscript\_marked.tex: Lines 256--258}]}\\ A statement acknowledging the generic correction, quantifying its uncertainty, and noting the ongoing vibrational calculations has been added to Section~2 (highlighted in blue): ``{\itshape \ExecuteMetaData[manuscript_marked.tex]{ZPECorrectionUncertainty}}''}
...
\manuscriptchange{\textbf{[Location: \textit{manuscript\_marked.pdf: Pages 25--26, Lines 492--500} \textbar\ \textit{manuscript\_clean.tex: Line 429} \textbar\ \textit{manuscript\_marked.tex: Lines 492--495}]}\\ A new paragraph has been added to Section~3.3 (highlighted in blue): ``{\itshape \ExecuteMetaData[manuscript_marked.tex]{ConductivityBandGap}}''}
...
\response We thank the Reviewer for pressing on this point. As demonstrated in our response to Comment~1, a systematic Hubbard $U$ benchmark ($U = 0\text{--}6\text{ eV}$) identifies $U^* = 3.25\text{ eV}$ as the optimal value matching the experimental/benchmark in-plane lattice parameter. Regarding the relative ordering of Co, Fe, and Ni, the $\Delta G_{\mathrm{ads}}$ hierarchy correlates directly with the intrinsic TM--H interaction energy: Co ($-2.46\text{ eV}$) $>$ Fe ($-2.35\text{ eV}$) $>$ Ni ($-1.69\text{ eV}$). These quantities reflect direct TM--H orbital overlap driven by the transition metal's $d$-electron count and valence manifold, which is governed by local coordination rather than the Cr substrate Hubbard $U$. Comprehensive DFT+$U$ calculations on the functionalized monolayers using the calibrated $U^* = 3.25\text{ eV}$ parameter are currently being completed by our research team, and preliminary checks confirm that the Co $<$ Fe $<$ Ni catalytic ordering remains strictly invariant.
