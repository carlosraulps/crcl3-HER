...
def update_all(root_dir):
    pattern = os.path.join(root_dir, "**", "job.sh")
    files = glob.glob(pattern, recursive=True)
    print(f"Found {len(files)} job.sh files in {root_dir}")
    for f in sorted(files):
        with open(f, 'r') as fp:
            content = fp.read()
        m = re.search(r'#SBATCH\s+-J\s+(\S+)', content)
        if m:
            job_name = m.group(1)
        else:
            job_name = "VASP_job"
        
        new_content = TEMPLATE.format(job_name=job_name)
        with open(f, 'w') as fp:
            fp.write(new_content)
        print(f"  ✓ Updated {f} (Job: {job_name})")

if __name__ == "__main__":
    import sys
    target = sys.argv[1] if len(sys.argv) > 1 else "/home/cr/simulations/crcl3-HER/crcl3-2x2-CoFeNi_ads-without-U"
    update_all(target)
