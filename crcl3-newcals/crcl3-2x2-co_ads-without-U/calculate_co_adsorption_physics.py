...
"""
================================================================================
 PHYSICAL & MATHEMATICAL JUSTIFICATION FOR COBALT ADSORPTION SITES AND
...
R_EQ_CRH = 1.5500 # Angstrom (Manuscript reported relaxed Co-Cr chemisorption distance)
...
# d(Co-Cl) = 2.25 A (slightly larger than gas-phase HCl 1.275 A by ~2%)
# Rationale:
...
DZ_S1 = 2.2500  # Angstrom above Cl11
...
# Placing H at dz = 1.90 A above the Cr central plane places H at z = 11.90 A, which is
...
DZ_S2 = 1.9000  # Angstrom above central slab plane (z = 0.5)
...
# Rationale:
#   - Exactly matches the manuscript equilibrium distance: d(Co-Cr) = 2.50 A.
#   - Distance to the 3 coordinating top-Cl ligands (Cl10, Cl11, Cl12):
#     d(Co-Cl) = sqrt(1.942^2 + 0.214^2) = 1.953 A >> r_cov(Co)+r_cov(Cl) = 1.33 A.
...
DZ_S3 = 2.5000  # Angstrom above Cr3
...
def analyze_h_cl_potential():
    """Analyze Co-Cl potential energy curve (HCl parameters: De ~ 4.62 eV, re = 1.275 A, a ~ 1.87 A^-1)."""
    de = 4.62
    re = 1.2746
    a = 1.87
...
def analyze_h_cr_potential():
    """Analyze Co-Cr potential energy curve (CrH parameters: De ~ 2.11 eV, re = 2.50 A, a ~ 1.90 A^-1)."""
    de = 2.11
    re = 1.5500
    a = 1.60
...
def main():
    print("=" * 85)
    print(" COBALT ADSORPTION PHYSICS & MATHEMATICAL ANALYSIS: CrCl3 (2x2) MONOLAYER")
    print("=" * 85)
    ...
    print("     Optimal Initial Co: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s1_cart))
    print("                        frac = ({:.5f}, {:.5f}, {:.5f})".format(*h_s1_frac))
    print("     Initial d(Co-Cl11): {:.4f} A".format(DZ_S1))
    ...
    print("     Optimal Initial Co: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s2_cart))
    ...
    print("     Optimal Initial Co: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s3_cart))
    print("                        frac = ({:.5f}, {:.5f}, {:.5f})".format(*h_s3_frac))
    print("     Initial d(Co-Cr3):  {:.4f} A".format(DZ_S3))
    ...
    print("     3D distance to coordinating Cl ligands:      d(Co-Cl) = {:.4f} A".format(d_h_cl))
    print("     Steric clearance: d(Co-Cl) - [r_cov(Co)+r_cov(Cl)] = {:.4f} A (No steric clash!)".format(
        d_h_cl - (R_COV_H + R_COV_CL)))

    print("\n3. MATHEMATICAL FORCE-DISTANCE ANALYSIS (Goldilocks Principle):")
    print("   -------------------------------------------------------------------------")
    print("   Target: Co-Cl interaction (Model: Morse Potential, De=4.62 eV, re=1.275 A)")
    print("   -------------------------------------------------------------------------")
    ...
    print("\n   -------------------------------------------------------------------------")
    print("   Target: Co-Cr interaction (Model: Morse Potential, De=2.11 eV, re=1.550 A)")
    print("   -------------------------------------------------------------------------")
    print("     r (A)   |  V(r) (eV)  |  F(r) (eV/A)  | Physical Interpretation")
    print("   ----------+-------------+---------------+---------------------------------")
    ...
    print("\n" + "=" * 85)
    print(" SUMMARY OF FINAL POSITIONS FOR setup_co_ads.py:")
    print("=" * 85)
