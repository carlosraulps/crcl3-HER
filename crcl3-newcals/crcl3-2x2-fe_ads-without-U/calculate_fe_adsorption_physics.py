...
"""
================================================================================
 PHYSICAL & MATHEMATICAL JUSTIFICATION FOR IRON ADSORPTION SITES AND
...
R_EQ_CRH = 1.5500 # Angstrom (Manuscript reported relaxed Fe-Cr chemisorption distance)
...
# d(Fe-Cl) = 2.25 A (slightly larger than gas-phase HCl 1.275 A by ~2%)
# Rationale:
...
# Rationale:
#   - Exactly matches the manuscript equilibrium distance: d(Fe-Cr) = 2.50 A.
#   - Distance to the 3 coordinating top-Cl ligands (Cl10, Cl11, Cl12):
#     d(Fe-Cl) = sqrt(1.942^2 + 0.214^2) = 1.953 A >> r_cov(Fe)+r_cov(Cl) = 1.33 A.
...
def analyze_h_cl_potential():
    """Analyze Fe-Cl potential energy curve (HCl parameters: De ~ 4.62 eV, re = 1.275 A, a ~ 1.87 A^-1)."""
    de = 4.62
    re = 1.2746
    a = 1.87
...
def analyze_h_cr_potential():
    """Analyze Fe-Cr potential energy curve (CrH parameters: De ~ 2.11 eV, re = 2.50 A, a ~ 1.90 A^-1)."""
    de = 2.11
    re = 1.5500
    a = 1.60
...
def main():
    print("=" * 85)
    print(" IRON ADSORPTION PHYSICS & MATHEMATICAL ANALYSIS: CrCl3 (2x2) MONOLAYER")
    print("=" * 85)
    ...
    print("     Optimal Initial Fe: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s1_cart))
    print("                        frac = ({:.5f}, {:.5f}, {:.5f})".format(*h_s1_frac))
    print("     Initial d(Fe-Cl11): {:.4f} A".format(DZ_S1))
    ...
    print("     Optimal Initial Fe: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s2_cart))
    ...
    print("     Optimal Initial Fe: cart = ({:.5f}, {:.5f}, {:.5f}) A".format(*h_s3_cart))
    print("                        frac = ({:.5f}, {:.5f}, {:.5f})".format(*h_s3_frac))
    print("     Initial d(Fe-Cr3):  {:.4f} A".format(DZ_S3))
    ...
    print("     3D distance to coordinating Cl ligands:      d(Fe-Cl) = {:.4f} A".format(d_h_cl))
    print("     Steric clearance: d(Fe-Cl) - [r_cov(Fe)+r_cov(Cl)] = {:.4f} A (No steric clash!)".format(
        d_h_cl - (R_COV_H + R_COV_CL)))

    print("\n3. MATHEMATICAL FORCE-DISTANCE ANALYSIS (Goldilocks Principle):")
    print("   -------------------------------------------------------------------------")
    print("   Target: Fe-Cl interaction (Model: Morse Potential, De=4.62 eV, re=1.275 A)")
    print("   -------------------------------------------------------------------------")
    ...
    print("\n   -------------------------------------------------------------------------")
    print("   Target: Fe-Cr interaction (Model: Morse Potential, De=2.11 eV, re=1.550 A)")
    print("   -------------------------------------------------------------------------")
    print("     r (A)   |  V(r) (eV)  |  F(r) (eV/A)  | Physical Interpretation")
    print("   ----------+-------------+---------------+---------------------------------")
    ...
    print("\n" + "=" * 85)
    print(" SUMMARY OF FINAL POSITIONS FOR setup_fe_ads.py:")
    print("=" * 85)
