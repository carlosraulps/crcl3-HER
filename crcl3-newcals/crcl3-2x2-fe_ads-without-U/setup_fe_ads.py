...
"""
================================================================================
 SETUP IRON ADSORPTION ON PRISTINE CrCl3 2x2 SUPERCELL (No Hubbard U)
================================================================================
 Generates VASP inputs for iron adsorption at three sites:
...
POTCAR_H = "/Users/apple/Research/abc/potentials/potpaw_PBE.64/Fe/POTCAR"

# Scratch base for SLURM jobs
SCRATCH_ROOT = "/home/cr/scratch_vasp/crcl3-2x2-fe_ads-without-U"
...
def generate_incar(vdw, site_name, a_2x2, has_tm):
    ...
    system_tag = "CrCl3 2x2 + Fe ({})".format(site_name) if has_tm else "CrCl3 2x2 Clean Substrate"
    magmom = "8*3.0 24*0.0 1*4.0" if has_tm else "8*3.0 24*0.0"
    magmom_comment = ("# Initial magnetic moments (muB): 3.0 for Cr3+ (d3, S=3/2), "
                      "0.0 for Cl- (closed shell)" +
                      (", 4.0 for Fe (d6 high-spin (4 unpaired electrons))" if has_tm else ""))
...
def setup_all():
    ...
    print("\n" + "=" * 85)
    print("   IRON ADSORPTION ON PRISTINE CrCl3 2x2 SUPERCELL (No Hubbard U)")
    ...
    for vdw_name, vdw_flag, vdw_desc in vdw_configs:
        ...
        job_name = "Fe_cln_{vdw_name}".format(vdw_name)
        ...
        for site_key, site_info in h_positions.items():
            ...
            write_poscar(
                ...
                "CrCl3 2x2 + Fe {} ({}, U=0) (a_2x2={:.4f}A, c={:.4f}A)".format(
                    site_info["label"], "D3" if vdw_flag else "no_vdw", a_2x2, C_VACUUM),
                lat_2x2, ["Cr", "Cl", "Fe"], [8, 24, 1],
                all_coords,
            )
            ...
            job_name = "Fe_{site_key}_{vdw_name}".format(site_key, vdw_name)
