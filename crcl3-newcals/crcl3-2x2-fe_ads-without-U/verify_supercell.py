...
"""
================================================================================
 VERIFICATION SCRIPT: Fe Adsorption VASP Inputs on CrCl3 2x2 Supercell
================================================================================
...
def verify_structure(poscar_path, contcar_1x1_path, has_tm, site_name):
    ...
    if has_tm:
        ...
        check(species == ["Cr", "Cl", "Fe"],
              "Species order: {} = ['Cr', 'Cl', 'Fe']".format(species),
              "Species order: {} != ['Cr', 'Cl', 'Fe']".format(species))
    else:
        check(counts == [8, 24],
    ...
    if has_tm:
        ...
        # Check Fe is above the slab (z > 0.5 in fractional)
        check(tm_coord[2] > 0.5,
              "Fe is above slab center (z_frac = {:.6f} > 0.5)".format(tm_coord[2]),
              "Fe is below slab center (z_frac = {:.6f})".format(tm_coord[2]))
        ...
        if site_name == "S1":
            ...
            check(d_xy < 1e-4,
                  "S1: Fe is positioned exactly atop Cl11 in-plane (d_xy = {:.6f} A)".format(d_xy),
                  "S1: H is not aligned atop Cl11 in-plane (d_xy = {:.6f} A)".format(d_xy))
            check(abs(d_tm_cl11 - 2.25) < 1e-3,
                  "S1: Fe-Cl11 distance = {:.4f} A (target 2.25 A)".format(d_tm_cl11),
                  "S1: Fe-Cl11 distance = {:.4f} A != 2.25 A".format(d_tm_cl11))
            ...
            check(d_xy < 1e-4,
                  "S2: Fe is positioned exactly at hollow site (0.5, 0.5) (d_xy = {:.6f} A)".format(d_xy),
                  "S2: H is not at hollow center (d_xy = {:.6f} A)".format(d_xy))
            check(abs(dz_slab - 1.90) < 1e-3,
                  "S2: Fe height above slab center = {:.4f} A (target 1.90 A)".format(dz_slab),
                  "S2: Fe height above slab center = {:.4f} A != 1.90 A".format(dz_slab))
            ...
            check(d_xy < 1e-4,
                  "S3: Fe is positioned exactly atop Cr3 in-plane (d_xy = {:.6f} A)".format(d_xy),
                  "S3: H is not aligned atop Cr3 in-plane (d_xy = {:.6f} A)".format(d_xy))
            check(abs(d_tm_cr3 - 2.50) < 1e-3,
                  "S3: Fe-Cr3 distance = {:.4f} A (matches paper equilibrium 2.50 A)".format(d_tm_cr3),
                  "S3: Fe-Cr3 distance = {:.4f} A != 2.50 A".format(d_tm_cr3))
...
def verify_potcar(potcar_path, has_tm):
    ...
    expected = ["Cr", "Cl", "Fe"] if has_tm else ["Cr", "Cl"]
...
def cross_validate_existing(new_poscar, existing_poscar):
    ...
    h_new = crd_new[-1]
    h_old = crd_old[-1]
    print("    [INFO] Fe position new (above Cr3): ({:.6f}, {:.6f}, {:.6f})".format(*h_new))
    print("    [INFO] Fe position old (above Cr1): ({:.6f}, {:.6f}, {:.6f})".format(*h_old))

    h_z_diff = abs(h_new[2] - h_old[2]) * C_VACUUM
    check(h_z_diff < 0.01,
          "Fe z-distance above Cr matches old calculation exactly: diff = {:.4f} A".format(h_z_diff),
          "Fe z-position diff = {:.3f} A (!= old calculation)".format(h_z_diff))
