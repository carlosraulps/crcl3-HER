...
"""
================================================================================
 SETUP NICKEL ADSORPTION ON PRISTINE CrCl3 2x2 SUPERCELL (No Hubbard U)
================================================================================
 Generates VASP inputs for nickel adsorption at three sites:
 ...
 Two dispersion variants:
   ...
   yes_vdw -- PBE + DFT-D3 (Grimme zero-damping, IVDW=12)
...
# POTCAR sources
POTCAR_CR = "/Users/apple/Research/abc/potentials/potpaw_PBE.64/Cr/POTCAR"
POTCAR_CL = "/Users/apple/Research/abc/potentials/potpaw_PBE.64/Cl/POTCAR"  # Cr + Cl
POTCAR_H = "/Users/apple/Research/abc/potentials/potpaw_PBE.64/Ni/POTCAR"

# Scratch base for SLURM jobs
SCRATCH_ROOT = "/home/cr/scratch_vasp/crcl3-2x2-ni_ads-without-U"
...
H_DIST_S1 = 2.25  # H above Atom 19 (Cl11) (S1 site; covalent H-Cl r_e ~ 1.275 A + 0.025 A)
H_DIST_S2 = 1.90  # H above slab center z=0.5 (S2 hollow site; 0.264 A above top Cl plane)
H_DIST_S3 = 2.50  # H above Atom 3 (Cr3) (S3 top-Cr site; exact manuscript equilibrium d(H-Cr)=1.55 A)
...
def generate_incar(vdw, site_name, a_2x2, has_tm):
    ...
    has_tm: bool -- whether H is present (affects MAGMOM and POTCAR order)
    """
    vdw_label = "D3" if vdw else "No vdW"
    system_tag = "CrCl3 2x2 + Ni ({})".format(site_name) if has_tm else "CrCl3 2x2 Clean Substrate"
    magmom = "8*3.0 24*0.0 1*2.0" if has_tm else "8*3.0 24*0.0"
    magmom_comment = ("# Initial magnetic moments (muB): 3.0 for Cr3+ (d3, S=3/2), "
                      "0.0 for Cl- (closed shell)" +
                      (", 2.0 for Ni (d8 high-spin (2 unpaired electrons))" if has_tm else ""))
    h_count_str = ", 1 H" if has_tm else ""
...
IVDW     = 12          # Grimme DFT-D3 zero-damping dispersion correction; models non-local vdW interactions
"""
    else:
        incar += """
# --- 5. van der Waals Dispersion Correction ---
...
def generate_job_script(job_name, project_dir, scratch_subdir):
    """Generate an optimized SLURM job script tailored for Huk cluster."""
    return """#!/bin/bash
#SBATCH -J {job_name}
#SBATCH -o job.%j.out
#SBATCH -e job.%j.err
...
# System: CrCl3 2x2 + Adatom ({job_name})
...
def setup_all():
    ...
    print("\n" + "=" * 85)
    print("   NICKEL ADSORPTION ON PRISTINE CrCl3 2x2 SUPERCELL (No Hubbard U)")
    ...
    for vdw_name, vdw_flag, vdw_desc in vdw_configs:
        ...
        job_name = "Ni_cln_{vdw_name}".format(vdw_name)
        ...
        for site_key, site_info in h_positions.items():
            ...
            write_poscar(
                ...
                "CrCl3 2x2 + Ni {} ({}, U=0) (a_2x2={:.4f}A, c={:.4f}A)".format(
                    site_info["label"], "D3" if vdw_flag else "no_vdw", a_2x2, C_VACUUM),
                lat_2x2, ["Cr", "Cl", "Ni"], [8, 24, 1],
                all_coords,
            )
            ...
            job_name = "Ni_{site_key}_{vdw_name}".format(site_key, vdw_name)
