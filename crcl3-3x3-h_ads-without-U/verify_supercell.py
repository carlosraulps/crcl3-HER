...
"""
================================================================================
 VERIFICATION SCRIPT: H Adsorption VASP Inputs on CrCl3 3x3 Supercell
================================================================================
"""
...
def verify_structure(poscar_path, contcar_1x1_path, has_h, site_name):
    ...
    check(abs(a_calc - a_ref) < 0.001,
          "a = {:.4f} A == 3 * 1x1 ref = {:.4f} A".format(a_calc, a_ref),
    ...
    expected_counts = [18, 54, 1] if has_h else [18, 54]
    check(counts == expected_counts, "Atom counts: {} == {}".format(counts, expected_counts), "Counts mismatch")

    # Min Cr-Cr distance
    cr_coords = coords[:18]
    min_crcr = min([min_image_dist(cr_coords[i], cr_coords[j], lat) for i in range(18) for j in range(i+1, 18)])
    check(abs(min_crcr - 3.4908) < 0.01,
          "Min Cr-Cr distance = {:.4f} A (expected 3.4908 A)".format(min_crcr),
          "Min Cr-Cr distance = {:.4f} A != 3.4908 A".format(min_crcr))

    # Avg Cr-Cl bond length (nearest neighbors < 3 A)
    crcl_dists = [min_image_dist(cr_coords[i], coords[18+j], lat) for i in range(18) for j in range(54)
                  if min_image_dist(cr_coords[i], coords[18+j], lat) < 3.0]
    ...
    if has_h:
        ...
        if site_name == "S1":
            # Target top-Cl
            cl_top = [coords[18+i] for i in range(54) if coords[18+i][2] > 0.5]
            target_cl = min(cl_top, key=lambda c: min_image_dist(h_coord[:2], c[:2], lat[:2, :2]))
            ...
            s2_center = [1.0/3.0, 1.0/3.0]
            d_xy = np.linalg.norm(((h_coord[:2] - s2_center) - np.round(h_coord[:2] - s2_center)) @ lat[:2, :2])
            dh = (h_coord[2] - 0.5) * C_VACUUM
            check(d_xy < 1e-4, "S2: H at hollow (1/3, 1/3) (d_xy = {:.6f} A)".format(d_xy), "S2: H not at hollow")
            check(abs(dh - 1.60) < 1e-3, "S2: H height above slab = {:.4f} A (target 1.60 A)".format(dh), "S2: dh != 1.60")
        elif site_name == "S3":
            target_cr = min(cr_coords, key=lambda c: min_image_dist(h_coord[:2], c[:2], lat[:2, :2]))
            d_xy = np.linalg.norm(((h_coord[:2] - target_cr[:2]) - np.round(h_coord[:2] - target_cr[:2])) @ lat[:2, :2])
            dh = (h_coord[2] - target_cr[2]) * C_VACUUM
            check(d_xy < 1e-4, "S3: H atop Cr in-plane (d_xy = {:.6f} A)".format(d_xy), "S3: H not aligned")
...
def main():
    ...
    print("\n" + "=" * 85)
    print("   VERIFICATION: H ADSORPTION VASP INPUTS ON CrCl3 3x3 SUPERCELL")
    print("=" * 85)
    ...
    print("\n" + "=" * 85)
    print("  VERIFICATION SUMMARY (3x3)")
